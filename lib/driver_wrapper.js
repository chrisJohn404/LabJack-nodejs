...
var apiEdge = {
    'LJM_OpenAll': [
        'int', [
            ...
            ref.refType(ref.types.int),     //aErrors

        ]
    ]
};
// Add apiBase to apiEdge
extend(apiEdge, apiBase);

//create FFI'd versions of the liblabjackLJM library
var liblabjack;
var openAllIsLoaded = false;
try {
    liblabjack = ffi.Library(LIBRARY_LOC, apiEdge);
    openAllIsLoaded = true;
}
catch (int) {
    console.log("Could not load edge API. Degrading to base API.");
    liblabjack = ffi.Library(LIBRARY_LOC, apiBase);
}
