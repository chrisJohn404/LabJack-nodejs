...
var edgeAPI = {
    'LJM_OpenAll': [
        'int', [
            'int',                          //DeviceType
            'int',                          //ConnectionType
            ref.refType(ref.types.int),     //NumOpened
            ref.refType(ref.types.int),     //aHandles
            ref.refType(ref.types.int),     //NumErrors
            ref.refType(ref.types.int),     //ErrorHandle
            ref.refType(ref.types.CString), //Error
        ]
    ]
};
// Add baseAPI to edgeAPI
extend(edgeAPI, baseAPI);

//create FFI'd versions of the liblabjackLJM library
var liblabjack;
var openAllIsLoaded = false;
try {
    liblabjack = ffi.Library(LIBRARY_LOC, edgeAPI);
    openAllIsLoaded = true;
}
catch (int) {
    console.log("Could not load edge API. Degrading to base API.");
    liblabjack = ffi.Library(LIBRARY_LOC, baseAPI);
}
